/*
    ...
    Description: Inserts data into [payments].[Transactions]
...
PRINT 'Inserting data into [payments].[Transactions]...';
GO

IF NOT EXISTS (SELECT TOP 1 1 FROM [payments].[Transactions])
BEGIN
    SET IDENTITY_INSERT [payments].[Transactions] ON;

    INSERT [payments].[Transactions] ([TransactionID], [UserID], [VendorProfileID], [BookingID], [Amount], [FeeAmount], [NetAmount], [Currency], [Description], [StripeChargeID], [Status], [CreatedAt]) VALUES (1, 144, 138, 40, CAST(1000.00 AS Decimal(10, 2)), CAST(29.30 AS Decimal(10, 2)), CAST(970.70 AS Decimal(10, 2)), N'USD', N'Stripe Payment (verified)', N'pi_3SHpivFJtS8tRcsD1nKtpNRQ', N'succeeded', CAST(N'2025-10-13T17:40:15.473' AS DateTime));
    ...
    SET IDENTITY_INSERT [payments].[Transactions] OFF;

    PRINT 'Inserted 21 records into [payments].[Transactions].';
END
ELSE
BEGIN
    PRINT 'Table [payments].[Transactions] already contains data. Skipping.';
END
GO
