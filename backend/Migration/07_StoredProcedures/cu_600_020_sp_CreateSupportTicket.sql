/*
    ...
    Description: Creates the [admin].[sp_CreateSupportTicket] stored procedure
...
PRINT 'Creating stored procedure [admin].[sp_CreateSupportTicket]...';
GO
...
BEGIN
    SET NOCOUNT ON;
    DECLARE @TicketNumber NVARCHAR(20) = 'TKT-' + FORMAT(GETUTCDATE(), 'yyyyMMdd') + '-' + RIGHT('0000' + CAST((SELECT ISNULL(MAX(TicketID), 0) + 1 FROM admin.SupportTickets) AS NVARCHAR), 4);
    
    INSERT INTO admin.SupportTickets (TicketNumber, UserID, UserEmail, UserName, Subject, Description, Category, Priority, Source, ConversationID, Attachments)
...
PRINT 'Stored procedure [admin].[sp_CreateSupportTicket] created successfully.';
GO
