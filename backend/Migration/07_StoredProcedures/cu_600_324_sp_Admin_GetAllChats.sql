-- =============================================
-- Stored Procedure: admin.sp_GetAllChats
...
IF EXISTS (SELECT 1 FROM sys.procedures WHERE object_id = OBJECT_ID(N'[admin].[sp_GetAllChats]'))
    DROP PROCEDURE [admin].[sp_GetAllChats];
GO

CREATE PROCEDURE [admin].[sp_GetAllChats]
...
BEGIN
    ...
    SELECT 
        ...
        ISNULL((SELECT COUNT(*) FROM messages.Messages WHERE ConversationID = c.ConversationID AND IsRead = 0), 0) as UnreadCount,
        0 as IsFlagged
    ...
    ORDER BY 
        CASE WHEN (SELECT TOP 1 CreatedAt FROM messages.Messages WHERE ConversationID = c.ConversationID ORDER BY CreatedAt DESC) IS NULL 
             THEN c.CreatedAt 
             ELSE (SELECT TOP 1 CreatedAt FROM messages.Messages WHERE ConversationID = c.ConversationID ORDER BY CreatedAt DESC) 
        END DESC
