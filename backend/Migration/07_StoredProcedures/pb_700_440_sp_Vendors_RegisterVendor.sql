/*
    Migration Script: Create Stored Procedure [vendors].[sp_RegisterVendor]
...
PRINT 'Creating stored procedure [vendors].[sp_RegisterVendor]...';
GO

IF EXISTS (SELECT 1 FROM sys.procedures WHERE object_id = OBJECT_ID(N'[vendors].[sp_RegisterVendor]'))
    DROP PROCEDURE [vendors].[sp_RegisterVendor];
GO

CREATE PROCEDURE [vendors].[sp_RegisterVendor]
    @UserID INT,
...
PRINT 'Stored procedure [vendors].[sp_RegisterVendor] created successfully.';
GO
