-- =============================================
-- Stored Procedure: vendors.sp_Dashboard_GetSetupCounts
...
IF EXISTS (SELECT 1 FROM sys.procedures WHERE object_id = OBJECT_ID(N'[vendors].[sp_Dashboard_GetSetupCounts]'))
    DROP PROCEDURE [vendors].[sp_Dashboard_GetSetupCounts];
GO

CREATE PROCEDURE [vendors].[sp_Dashboard_GetSetupCounts]
    @VendorProfileID INT
AS
BEGIN
    SET NOCOUNT ON;
