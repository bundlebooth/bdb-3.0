/*
    Migration Script: Create Stored Procedure [forum].[sp_Vote]
    Description: Creates the [forum].[sp_Vote] stored procedure
*/
...
IF EXISTS (SELECT 1 FROM sys.procedures WHERE object_id = OBJECT_ID(N'[forum].[sp_Vote]'))
    DROP PROCEDURE [forum].[sp_Vote];
GO
...
CREATE   PROCEDURE [forum].[sp_Vote]
    @UserID INT,
    @PostID INT = NULL,
    @CommentID INT = NULL,
