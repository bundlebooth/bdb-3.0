/*
    ...
    Description: Creates the [dbo].[sp_GetVendorSetupProgress] stored procedure
...
PRINT 'Creating stored procedure [dbo].[sp_GetVendorSetupProgress]...';
GO

IF EXISTS (SELECT 1 FROM sys.procedures WHERE object_id = OBJECT_ID(N'[dbo].[sp_GetVendorSetupProgress]'))
    DROP PROCEDURE [dbo].[sp_GetVendorSetupProgress];
GO
...
PRINT 'Stored procedure [dbo].[sp_GetVendorSetupProgress] created successfully.';
GO
