...
BEGIN
    ...
    ORDER BY 
        CASE @SortBy
            WHEN 'price-low' THEN f.MinPrice
            WHEN 'price-high' THEN -f.MinPrice
            WHEN 'rating' THEN -f.AverageRating
            WHEN 'popular' THEN -f.FavoriteCount
            WHEN 'nearest' THEN ISNULL(f.DistanceMiles, 999999)
            ELSE f.BusinessName
        END
    OFFSET @Offset ROWS
