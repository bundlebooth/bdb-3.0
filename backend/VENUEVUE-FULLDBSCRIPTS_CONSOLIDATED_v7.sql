...
-- UPDATED: Added Request Management System with 24-hour expiration and approval workflow
...
-- Booking Requests (UPDATED: Enhanced for request management system with 24-hour expiration)
CREATE TABLE BookingRequests (
    RequestID INT PRIMARY KEY IDENTITY(1,1),
    UserID INT FOREIGN KEY REFERENCES Users(UserID),
    VendorProfileID INT FOREIGN KEY REFERENCES VendorProfiles(VendorProfileID),
    Services NVARCHAR(MAX), -- JSON array of requested services
    EventDate DATE,
    EventTime TIME,
    ...
    Status NVARCHAR(50) DEFAULT 'pending', -- pending, approved, declined, expired, cancelled
    ResponseMessage NVARCHAR(MAX),
    ProposedPrice DECIMAL(10, 2),
    CreatedAt DATETIME DEFAULT GETDATE(),
    ExpiresAt DATETIME NOT NULL, -- 24 hours from creation
    RespondedAt DATETIME,
    INDEX IX_BookingRequests_Status_ExpiresAt (Status, ExpiresAt),
    INDEX IX_BookingRequests_VendorProfileID_Status (VendorProfileID, Status)
);
GO
...
CREATE TABLE Bookings (
    ...
    UserID INT FOREIGN KEY REFERENCES Users(UserID),
    VendorProfileID INT FOREIGN KEY REFERENCES VendorProfiles(VendorProfileID),
    RequestID INT FOREIGN KEY REFERENCES BookingRequests(RequestID),
    ServiceID INT FOREIGN KEY REFERENCES Services(ServiceID),
...
-- Chat conversations (UPDATED: Added RequestID for request management system)
...
-- Insert sample booking requests (UPDATED: Added for request management system)
INSERT INTO BookingRequests (UserID, VendorProfileID, Services, EventDate, EventTime, EventLocation, AttendeeCount, Budget, SpecialRequests, Status, ExpiresAt)
VALUES
(1, 1, '["Wedding Venue", "Catering"]', '2024-06-15', '14:00:00', 'Central Park, New York', 150, 15000.00, 'Need vegetarian options and outdoor ceremony setup', 'pending', DATEADD(HOUR, 24, GETDATE())),
(2, 2, '["Photography"]', '2024-05-20', '10:00:00', 'Brooklyn Bridge', 2, 2500.00, 'Engagement photos with city skyline backdrop', 'approved', DATEADD(HOUR, 24, GETDATE())),
(3, 3, '["Corporate Catering"]', '2024-04-10', '12:00:00', 'Manhattan Office Building', 50, 3000.00, 'Business lunch for quarterly meeting', 'declined', DATEADD(HOUR, 24, GETDATE()));
GO

-- Insert conversations (UPDATED: Added RequestID support)
INSERT INTO Conversations (UserID, VendorProfileID, BookingID, RequestID, Subject, LastMessageAt)
VALUES
(1, 1, 1, NULL, 'Wedding Booking #1', '2023-02-20'),
(1, 2, 2, NULL, 'Photography Booking #2', '2023-02-15'),
(2, 3, 3, NULL, 'Corporate Event #3', '2023-02-18'),
(3, 1, 4, NULL, 'Wedding Booking #4', '2023-02-10'),
(4, 2, 5, NULL, 'Engagement Session #5', '2023-02-05'),
(1, 1, NULL, 1, 'Wedding Request Discussion', GETDATE()),
(2, 2, NULL, 2, 'Photography Request Discussion', GETDATE()),
(3, 3, NULL, 3, 'Catering Request Discussion', GETDATE());
GO
...
VALUES
(1, 1, 1, 1250.00, 62.50, 1187.50, 'Wedding deposit payment', 'ch_123456789', 'completed'),
(1, 2, 2, 2500.00, 125.00, 2375.00, 'Photography full payment', 'ch_987654321', 'completed'),
(2, 3, 3, 1500.00, 75.00, 1425.00, 'Corporate event deposit', 'ch_456789123', 'completed'),
(3, 1, 4, 2400.00, 120.00, 2280.00, 'Wedding deposit payment', 'ch_789123456', 'completed'),
(4, 2, 5, 350.00, 17.50, 332.50, 'Engagement session payment', 'ch_321654987', 'completed');
GO
...
-- Create conversation with request support
IF OBJECT_ID('sp_CreateConversation', 'P') IS NOT NULL
    DROP PROCEDURE sp_CreateConversation;
GO
...
CREATE PROCEDURE sp_CreateConversation
    ...
    @BookingID INT = NULL,
    @RequestID INT = NULL,
    @Subject NVARCHAR(255) = 'New Conversation'
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        INSERT INTO Conversations (UserID, VendorProfileID, BookingID, RequestID, Subject, CreatedAt)
        VALUES (@UserID, @VendorProfileID, @BookingID, @RequestID, @Subject, GETDATE());
...
IF OBJECT_ID('sp_GetVendorPendingRequests', 'P') IS NOT NULL
    DROP PROCEDURE sp_GetVendorPendingRequests;
GO

CREATE PROCEDURE sp_GetVendorPendingRequests
    @VendorProfileID INT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    SELECT 
        ...
        br.ExpiresAt,
        c.ConversationID,
        DATEDIFF(HOUR, GETDATE(), br.ExpiresAt) as HoursRemaining,
        DATEDIFF(MINUTE, GETDATE(), br.ExpiresAt) as MinutesRemaining
    FROM BookingRequests br
    LEFT JOIN Users u ON br.UserID = u.UserID
    LEFT JOIN Conversations c ON br.RequestID = c.RequestID
...
IF OBJECT_ID('sp_GetUserRequestHistory', 'P') IS NOT NULL
    DROP PROCEDURE sp_GetUserRequestHistory;
GO

CREATE PROCEDURE sp_GetUserRequestHistory
    @UserID INT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    SELECT 
        ...
        c.ConversationID,
        CASE 
            WHEN br.ExpiresAt < GETDATE() AND br.Status = 'pending' THEN 'expired'
            ELSE br.Status 
        END as CurrentStatus,
        CASE 
            WHEN br.Status = 'pending' AND br.ExpiresAt > GETDATE() THEN DATEDIFF(HOUR, GETDATE(), br.ExpiresAt)
            ELSE NULL
...
PRINT 'VenueVue Database Schema with Request Management System completed successfully.';
