...
BEGIN
    ...
    -- Vendor images as JSON array (matching sp_SearchVendors format)
    SELECT 
        COALESCE(
            (SELECT 
                '[' + STRING_AGG(
                    '{"url":"' + vi.ImageURL + 
                    '","isPrimary":' + CASE WHEN vi.IsPrimary = 1 THEN 'true' ELSE 'false' END + 
                    ',"displayOrder":' + CAST(vi.DisplayOrder AS NVARCHAR(10)) + 
                    ',"caption":"' + ISNULL(vi.Caption, '') + '"}', ','
                ) WITHIN GROUP (ORDER BY vi.IsPrimary DESC, vi.DisplayOrder ASC) + ']'
             FROM VendorImages vi 
             WHERE vi.VendorProfileID = @VendorProfileID),
            '[]'
        ) as images;
...
BEGIN
    ...
    -- Main vendor search query with images arrays included
    SELECT 
        vp.VendorProfileID as id,
        vp.BusinessName as name,
        vp.BusinessDescription as description,
        vp.BusinessPhone as phone,
        vp.BusinessEmail as email,
        ...
        vp.Capacity,
        vp.Rooms,
        vp.FeaturedImageURL as featuredImage,
        
        -- Calculate average rating
        COALESCE((SELECT AVG(CAST(r.Rating AS FLOAT)) FROM Reviews r WHERE r.VendorProfileID = vp.VendorProfileID), 0) as rating,
        
        -- Count total bookings
        COALESCE((SELECT COUNT(*) FROM Bookings b WHERE b.VendorProfileID = vp.VendorProfileID), 0) as BookingCount,
        
        -- Categories as comma-separated string
        COALESCE((SELECT STRING_AGG(vc.Category, ', ') FROM VendorCategories vc WHERE vc.VendorProfileID = vp.VendorProfileID), '') as Categories,
        
        -- Images as JSON array - KEY FEATURE: NO MORE JS QUERIES NEEDED
        COALESCE(
            (SELECT 
                '[' + STRING_AGG(
                    '{"url":"' + vi.ImageURL + 
             ...
             FROM VendorImages vi 
             WHERE vi.VendorProfileID = vp.VendorProfileID),
            '[]'
        ) as images,
    ...
    AND (@SearchTerm IS NULL OR 
         ...
         vp.BusinessDescription LIKE '%' + @SearchTerm + '%' OR
         EXISTS (SELECT 1 FROM VendorCategories vc WHERE vc.VendorProfileID = vp.VendorProfileID AND vc.Category LIKE '%' + @SearchTerm + '%'))
    
    -- Category filter
    AND (@Category IS NULL OR 
         EXISTS (SELECT 1 FROM VendorCategories vc WHERE vc.VendorProfileID = vp.VendorProfileID AND vc.Category = @Category))
    ...
    ORDER BY 
        CASE WHEN @SortBy = 'newest' THEN vp.VendorProfileID END DESC,
        CASE WHEN @SortBy = 'rating' THEN (SELECT AVG(CAST(r.Rating AS FLOAT)) FROM Reviews r WHERE r.VendorProfileID = vp.VendorProfileID) END DESC,
        -- Default: recommended (premium first, then by ID)
        vp.IsPremium DESC, vp.VendorProfileID DESC
