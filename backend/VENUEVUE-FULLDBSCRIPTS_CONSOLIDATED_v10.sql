...
CREATE TABLE VendorProfiles (
    ...
    -- Visibility Control (binary flag for admin control of main grid display)
    IsVisible BIT DEFAULT 0, -- 0 = hidden from main grid, 1 = visible on main grid
    CreatedAt DATETIME DEFAULT GETDATE(),
    UpdatedAt DATETIME DEFAULT GETDATE()
);
GO
...
-- Create index for IsVisible (for fast vendor discovery queries)
CREATE INDEX IX_VendorProfiles_IsVisible 
ON VendorProfiles(IsVisible)
WHERE IsVisible = 1;
GO
...
-- Updated to support account type selection (client/vendor)
...
BEGIN
    ...
    WITH FilteredVendors AS (
        SELECT 
            ...
            (SELECT COUNT(*) FROM Favorites f WHERE f.VendorProfileID = v.VendorProfileID) AS FavoriteCount,
            (SELECT COUNT(*) FROM Bookings b WHERE b.VendorProfileID = v.VendorProfileID) AS BookingCount,
            -- Average response time in minutes (for Responsive Vendors section)
            (SELECT AVG(DATEDIFF(MINUTE, m.CreatedAt, m2.CreatedAt)) 
             FROM Messages m 
             JOIN Messages m2 ON m.ConversationID = m2.ConversationID AND m2.SenderID = v.UserID AND m2.CreatedAt > m.CreatedAt
             WHERE m.ReceiverID = v.UserID AND m.SenderID != v.UserID
             AND m2.MessageID = (SELECT TOP 1 MessageID FROM Messages WHERE ConversationID = m.ConversationID AND SenderID = v.UserID AND CreatedAt > m.CreatedAt ORDER BY CreatedAt ASC)
        ...
        WHERE u.IsActive = 1
        AND v.IsCompleted = 1
        AND v.IsVisible = 1';  -- Only show vendors that are visible (admin-controlled)
...
-- UPDATED: Add/Update Vendor Service with Unified Pricing Support
-- This version supports both legacy CategoryName-based calls and new unified pricing
...
-- ==============================================
-- VENDOR FEATURES/QUESTIONNAIRE SYSTEM
-- Allows vendors to select features that describe their services
-- Organized by category with icons for visual display
-- ==============================================

-- Master table for vendor feature categories
CREATE TABLE VendorFeatureCategories (
    CategoryID INT PRIMARY KEY IDENTITY(1,1),
    CategoryName NVARCHAR(100) NOT NULL UNIQUE,
    CategoryIcon NVARCHAR(50), -- Lucide icon name
    ApplicableVendorCategories NVARCHAR(500), -- Comma-separated list of applicable vendor categories (e.g., 'beauty,cake,catering' or 'all')
    DisplayOrder INT DEFAULT 0,
    IsActive BIT DEFAULT 1,
    CreatedAt DATETIME DEFAULT GETDATE()
);
...
-- Master table for vendor features organized by category
CREATE TABLE VendorFeatures (
    FeatureID INT PRIMARY KEY IDENTITY(1,1),
    CategoryID INT FOREIGN KEY REFERENCES VendorFeatureCategories(CategoryID),
    FeatureName NVARCHAR(100) NOT NULL,
    FeatureDescription NVARCHAR(500),
    FeatureIcon NVARCHAR(50), -- Lucide icon name
    DisplayOrder INT DEFAULT 0,
    IsActive BIT DEFAULT 1,
    CreatedAt DATETIME DEFAULT GETDATE(),
    CONSTRAINT UC_CategoryFeature UNIQUE (CategoryID, FeatureName)
);
GO

-- Junction table for vendor feature selections
CREATE TABLE VendorSelectedFeatures (
    VendorFeatureSelectionID INT PRIMARY KEY IDENTITY(1,1),
    VendorProfileID INT FOREIGN KEY REFERENCES VendorProfiles(VendorProfileID) ON DELETE CASCADE,
    FeatureID INT FOREIGN KEY REFERENCES VendorFeatures(FeatureID),
    CreatedAt DATETIME DEFAULT GETDATE(),
    CONSTRAINT UC_VendorFeature UNIQUE (VendorProfileID, FeatureID)
);
GO

-- Indexes for performance
CREATE INDEX IX_VendorFeatures_CategoryID ON VendorFeatures(CategoryID);
CREATE INDEX IX_VendorSelectedFeatures_VendorProfileID ON VendorSelectedFeatures(VendorProfileID);
...
CREATE OR ALTER PROCEDURE sp_GetVendorSelectedFeatures
    @VendorProfileID INT
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE OR ALTER PROCEDURE sp_SaveVendorFeatureSelections
    @VendorProfileID INT,
...
CROSS APPLY (VALUES
    -- Venue Features
    ('Indoor Ceremony Space', 'Climate-controlled indoor ceremony area', 'church', 1),
    ('Outdoor Ceremony Space', 'Beautiful outdoor ceremony setting', 'trees', 2),
    ('On-Site Catering', 'Professional catering services available', 'chef-hat', 3),
    ('Wheelchair Accessible', 'Full accessibility for all guests', 'accessibility', 4),
    ('Parking Available', 'Convenient on-site parking', 'car-front', 5),
    ('Sound System Included', 'Professional audio equipment', 'speaker', 6),
    ('WiFi Available', 'Complimentary high-speed internet', 'wifi', 7),
    ('Garden/Outdoor Space', 'Scenic outdoor areas for photos', 'trees', 8),
    ('Scenic Views', 'Stunning panoramic views', 'eye', 9),
    ('Dance Floor', 'Dedicated space for dancing', 'disc', 10),
    ('Stage/Platform', 'Elevated stage for performances', 'presentation', 11),
    ('Private Dressing Rooms', 'Comfortable preparation areas', 'door-closed', 12),
    ('Overnight Accommodations', 'On-site lodging available', 'bed', 13),
    ('Airport Shuttle', 'Transportation to/from airport', 'plane', 14),
    ('Vendor Preferred List', 'Curated list of trusted vendors', 'users', 15),
    ('Exclusive Use', 'Private venue rental', 'camera-off', 16),
    ('Power Backup', 'Generator for uninterrupted service', 'zap', 17)
) v(FeatureName, FeatureDescription, FeatureIcon, DisplayOrder)
WHERE c.CategoryName = 'Venue Features';
GO

-- Insert photography & video features
INSERT INTO VendorFeatures (CategoryID, FeatureName, FeatureDescription, FeatureIcon, DisplayOrder)
SELECT c.CategoryID, v.FeatureName, v.FeatureDescription, v.FeatureIcon, v.DisplayOrder
FROM VendorFeatureCategories c
CROSS APPLY (VALUES
    ('Engagement Session', 'Pre-wedding photo session included', 'heart', 1),
    ('Second Photographer', 'Additional photographer for coverage', 'camera', 2),
    ('Photo Booth', 'Fun interactive photo booth', 'images', 3),
    ('Same-Day Edit', 'Quick highlight video on wedding day', 'zap', 4),
    ('Drone Footage', 'Aerial photography and video', 'plane', 5),
    ('Photo Albums', 'Professional printed albums', 'book-open', 6),
    ('Digital Downloads', 'High-resolution digital files', 'file', 7),
    ('Online Gallery', 'Private online viewing gallery', 'images', 8),
    ('Print Rights', 'Full rights to print photos', 'printer', 9),
    ('Cinematic Video', 'Movie-style wedding film', 'film', 10),
    ('Live Streaming', 'Stream ceremony to remote guests', 'radio', 11)
) v(FeatureName, FeatureDescription, FeatureIcon, DisplayOrder)
WHERE c.CategoryName = 'Photography & Video';
GO

-- Insert music & entertainment features
INSERT INTO VendorFeatures (CategoryID, FeatureName, FeatureDescription, FeatureIcon, DisplayOrder)
SELECT c.CategoryID, v.FeatureName, v.FeatureDescription, v.FeatureIcon, v.DisplayOrder
FROM VendorFeatureCategories c
CROSS APPLY (VALUES
    ('Live Band', 'Professional live music performance', 'guitar', 1),
    ('DJ Services', 'Professional DJ and MC', 'disc', 2),
    ('Ceremony Music', 'Music for ceremony and processional', 'music', 3),
    ('Sound Equipment', 'Professional audio system', 'speaker', 4),
    ('Lighting Package', 'Mood and dance floor lighting', 'lightbulb', 5),
    ('Microphones', 'Wireless microphones for speeches', 'mic', 6),
    ('Music Requests', 'Custom playlist and requests', 'list-music', 7),
    ('Cocktail Hour Music', 'Entertainment during cocktail hour', 'glass-water', 8),
    ('Karaoke', 'Interactive karaoke setup', 'mic-vocal', 9),
    ('String Quartet', 'Classical music ensemble', 'music', 10),
    ('Acoustic Guitarist', 'Solo acoustic performance', 'guitar', 11)
) v(FeatureName, FeatureDescription, FeatureIcon, DisplayOrder)
WHERE c.CategoryName = 'Music & Entertainment';
GO

-- Insert catering & bar features
INSERT INTO VendorFeatures (CategoryID, FeatureName, FeatureDescription, FeatureIcon, DisplayOrder)
SELECT c.CategoryID, v.FeatureName, v.FeatureDescription, v.FeatureIcon, v.DisplayOrder
FROM VendorFeatureCategories c
CROSS APPLY (VALUES
    ('Full Bar Service', 'Complete bar with bartenders', 'wine', 1),
    ('Signature Cocktails', 'Custom craft cocktails', 'glass-water', 2),
    ('Wine Service', 'Curated wine selection', 'wine', 3),
    ('Beer Selection', 'Craft and domestic beers', 'beer', 4),
    ('Non-Alcoholic Options', 'Mocktails and soft drinks', 'glass-water', 5),
    ('Buffet Style', 'Self-service buffet setup', 'utensils', 6),
    ('Plated Dinner', 'Formal served dinner', 'utensils-crossed', 7),
    ('Family Style', 'Shared platters at tables', 'users', 8),
    ('Cocktail Hour Appetizers', 'Passed hors d''oeuvres', 'arrow-right-circle', 9),
    ('Dessert Bar', 'Variety of desserts', 'cake', 10),
    ('Coffee & Tea Service', 'Hot beverage station', 'coffee', 11),
    ('Vegan Options', 'Plant-based menu choices', 'leaf', 12),
    ('Gluten-Free Options', 'Gluten-free menu available', 'wheat-off', 13),
    ('Kids Menu', 'Special children''s meals', 'baby', 14),
    ('Late Night Snacks', 'Evening food service', 'scroll-text', 15),
    ('Cake Cutting Service', 'Professional cake service', 'cake', 16)
) v(FeatureName, FeatureDescription, FeatureIcon, DisplayOrder)
WHERE c.CategoryName = 'Catering & Bar';
GO

-- Insert floral & decor features
INSERT INTO VendorFeatures (CategoryID, FeatureName, FeatureDescription, FeatureIcon, DisplayOrder)
SELECT c.CategoryID, v.FeatureName, v.FeatureDescription, v.FeatureIcon, v.DisplayOrder
FROM VendorFeatureCategories c
CROSS APPLY (VALUES
    ('Bridal Bouquet', 'Custom bridal bouquet', 'flower', 1),
    ('Ceremony Flowers', 'Altar and aisle arrangements', 'flower-2', 2),
    ('Reception Centerpieces', 'Table centerpiece designs', 'flower', 3),
    ('Boutonnieres', 'Flowers for groomsmen', 'flower', 4),
    ('Corsages', 'Wrist and pin corsages', 'flower', 5),
    ('Arch/Chuppah Decor', 'Ceremony structure florals', 'hexagon', 6),
    ('Aisle Decorations', 'Aisle markers and petals', 'rainbow', 7),
    ('Sweetheart Table Decor', 'Head table arrangements', 'heart', 8),
    ('Uplighting', 'Ambient wall lighting', 'lamp', 9),
    ('Draping', 'Fabric ceiling treatments', 'cloud', 10),
    ('Linens & Napkins', 'Table linens and napkins', 'layers', 11),
    ('Chiavari Chairs', 'Elegant chair rentals', 'armchair', 12),
    ('Charger Plates', 'Decorative plate chargers', 'circle', 13),
    ('Candles & Votives', 'Ambient candle lighting', 'flame', 14),
    ('Signage', 'Welcome and directional signs', 'signpost', 15),
    ('Table Numbers', 'Numbered table markers', 'trending-up', 16)
) v(FeatureName, FeatureDescription, FeatureIcon, DisplayOrder)
WHERE c.CategoryName = 'Floral & Decor';
GO

-- Insert event services features
INSERT INTO VendorFeatures (CategoryID, FeatureName, FeatureDescription, FeatureIcon, v.DisplayOrder)
SELECT c.CategoryID, v.FeatureName, v.FeatureDescription, v.FeatureIcon, v.DisplayOrder
FROM VendorFeatureCategories c
CROSS APPLY (VALUES
    ('Day-of Coordination', 'Professional event coordination', 'clipboard-check', 1),
    ('Full Planning Service', 'Complete wedding planning', 'clipboard-list', 2),
    ('Vendor Coordination', 'Liaison with all vendors', 'users-round', 3),
    ('Timeline Creation', 'Detailed event timeline', 'calendar-days', 4),
    ('Setup & Breakdown', 'Event setup and cleanup', 'hand', 5),
    ('Rehearsal Coordination', 'Wedding rehearsal management', 'calendar-check', 6),
    ('Guest Services', 'Concierge for guests', 'users', 7),
    ('Officiant Services', 'Licensed officiant available', 'handshake', 8),
    ('Budget Management', 'Financial planning assistance', 'dollar-sign', 9)
) v(FeatureName, FeatureDescription, FeatureIcon, DisplayOrder)
WHERE c.CategoryName = 'Event Services';
GO

-- Insert transportation features
INSERT INTO VendorFeatures (CategoryID, FeatureName, FeatureDescription, FeatureIcon, DisplayOrder)
SELECT c.CategoryID, v.FeatureName, v.FeatureDescription, v.FeatureIcon, v.DisplayOrder
FROM VendorFeatureCategories c
CROSS APPLY (VALUES
    ('Limousine Service', 'Luxury limo transportation', 'car', 1),
    ('Vintage Car Rental', 'Classic car for photos', 'car-front', 2),
    ('Guest Shuttle', 'Transportation for guests', 'bus', 3),
    ('Valet Parking', 'Professional valet service', 'key-round', 4),
    ('Trolley Service', 'Charming trolley transport', 'bus-front', 5)
) v(FeatureName, FeatureDescription, FeatureIcon, DisplayOrder)
...
FROM VendorFeatureCategories c
CROSS APPLY (VALUES
    ('Sparkler Send-Off', 'Sparklers for grand exit', 'sparkles', 1),
    ('Fireworks Display', 'Professional fireworks show', 'flame-kindling', 2),
    ('Ice Sculpture', 'Custom ice sculptures', 'flame', 3),
    ('Champagne Tower', 'Elegant champagne display', 'wine', 4),
    ('Cigar Bar', 'Premium cigar station', 'flame', 5),
    ('Kids Entertainment', 'Activities for children', 'gamepad-2', 6),
    ('Pet Services', 'Accommodations for pets', 'heart', 7),
    ('Cultural Ceremony', 'Traditional ceremony elements', 'drama', 8),
    ('Magic/Entertainer', 'Professional entertainment', 'wand', 9)
) v(FeatureName, FeatureDescription, FeatureIcon, DisplayOrder)
WHERE c.CategoryName = 'Special Features';
GO

-- Insert event planning features
INSERT INTO VendorFeatures (CategoryID, FeatureName, FeatureDescription, FeatureIcon, DisplayOrder)
SELECT c.CategoryID, v.FeatureName, v.FeatureDescription, v.FeatureIcon, v.DisplayOrder
FROM VendorFeatureCategories c
CROSS APPLY (VALUES
    ('Unlimited Meetings', 'Unlimited planning consultations', 'calendar-check', 1),
    ('Design Consultation', 'Professional design advice', 'palette', 2),
    ('Vendor Recommendations', 'Curated vendor suggestions', 'users', 3),
    ('Contract Review', 'Legal contract assistance', 'file', 4),
    ('RSVP Management', 'Guest response tracking', 'clipboard-check', 5),
    ('Seating Chart', 'Table assignment planning', 'map-pin', 6),
    ('Digital Tools Access', 'Online planning platform', 'wifi', 7),
...
-- ============================================
-- UNIFIED VENDOR QUERY SYSTEM
-- ============================================
-- ALL vendor queries now use sp_SearchVendors as the SINGLE source of truth.
-- 
-- The /vendors endpoint with includeDiscoverySections=true returns:
--   - vendors: Main grid results
--   - discoverySections: Categorized sections (Trending, Top Rated, Premium, etc.)
-- 
-- All from ONE query that filters by IsVisible = 1 for admin visibility control.
--
-- Frontend call: GET /api/vendors?includeDiscoverySections=true&pageSize=50
--
-- Discovery sections are generated in the API by sorting/filtering the query results:
--   - Trending: sorted by booking count
--   - Top Rated: filtered by rating >= 4.0, sorted by rating
--   - Premium: filtered by isPremium = true
--   - Budget-Friendly: sorted by price low
--   - Nearby: sorted by distance (if lat/lng provided)
--   - Most Reviewed: sorted by review count
--
-- NO SEPARATE STORED PROCEDURES NEEDED - sp_SearchVendors handles everything!
-- ============================================

PRINT '============================================='
PRINT '✅ UNIFIED VENDOR QUERY SYSTEM'
PRINT '============================================='
PRINT 'sp_SearchVendors is the SINGLE source of truth for all vendor queries.'
PRINT 'Use /api/vendors?includeDiscoverySections=true to get grid + sections.'
...
BEGIN
    ...
    ('How do I book a vendor?', 'Browse vendors on our platform, select one you like, choose your date and time, and complete the booking process. You''ll receive a confirmation email once the vendor accepts.', 'Booking', 1, 1),
    ('What is the cancellation policy?', 'Cancellation policies vary by vendor. Check the specific vendor''s profile for their cancellation terms. Generally, cancellations made 48+ hours in advance receive a full refund.', 'Booking', 2, 1),
    ('How do payments work?', 'Payments are processed securely through Stripe. You pay when booking, and the vendor receives payment after the service is completed, minus our platform fee.', 'Payments', 3, 1),
    ('How do I become a vendor?', 'Click "Become a Vendor" in the navigation, complete the registration form, set up your profile, connect your Stripe account, and start receiving bookings!', 'Vendors', 4, 1);
    PRINT 'Inserted default FAQs';
END
GO
...
-- =============================================
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'CommissionSettings')
BEGIN
    CREATE TABLE CommissionSettings (
        ...
        Description NVARCHAR(500),
        SettingType NVARCHAR(50) DEFAULT 'percentage', -- percentage, fixed, boolean
    ...
    PRINT 'Created CommissionSettings table';
END
GO

-- Insert default commission settings
IF NOT EXISTS (SELECT 1 FROM CommissionSettings)
BEGIN
    INSERT INTO CommissionSettings (SettingKey, SettingValue, Description, SettingType, MinValue, MaxValue, IsActive) VALUES
    ('platform_commission_rate', '15', 'Platform commission percentage taken from vendor payouts', 'percentage', 0, 50, 1),
    ('renter_processing_fee_rate', '5', 'Processing fee percentage charged to renters/customers', 'percentage', 0, 20, 1),
    ('minimum_booking_amount', '25', 'Minimum booking amount in dollars', 'fixed', 0, 1000, 1),
    ('stripe_application_fee_rate', '2.9', 'Stripe''s base processing fee percentage', 'percentage', 0, 10, 1),
    ('stripe_fixed_fee', '0.30', 'Stripe''s fixed fee per transaction in dollars', 'fixed', 0, 5, 1),
    ('instant_payout_enabled', 'false', 'Whether vendors can request instant payouts', 'boolean', NULL, NULL, 1),
    ('payout_delay_days', '7', 'Number of days to hold funds before vendor payout', 'fixed', 0, 30, 1);
    PRINT 'Inserted default commission settings';
END
GO
...
BEGIN
    CREATE TABLE PaymentTransactions (
        ...
        BookingID INT,
        UserID INT, -- Customer who paid
        VendorProfileID INT, -- Vendor receiving payment
        
        -- Amounts
        GrossAmount DECIMAL(10,2) NOT NULL, -- Total amount customer paid
        PlatformFee DECIMAL(10,2) NOT NULL, -- Our commission
        ProcessingFee DECIMAL(10,2) NOT NULL, -- Stripe fees
        VendorPayout DECIMAL(10,2) NOT NULL, -- What vendor receives
        
        -- Stripe IDs
        StripePaymentIntentID NVARCHAR(255),
        StripeTransferID NVARCHAR(255),
        StripeChargeID NVARCHAR(255),
        
        -- Status
        Status NVARCHAR(50) DEFAULT 'pending', -- pending, completed, refunded, failed
        PayoutStatus NVARCHAR(50) DEFAULT 'pending', -- pending, scheduled, paid, failed
...
IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_PaymentTransactions_BookingID')
BEGIN
    CREATE INDEX IX_PaymentTransactions_BookingID ON PaymentTransactions(BookingID);
    CREATE INDEX IX_PaymentTransactions_VendorProfileID ON PaymentTransactions(VendorProfileID);
    CREATE INDEX IX_PaymentTransactions_Status ON PaymentTransactions(Status);
    PRINT 'Created PaymentTransactions indexes';
END
GO
