  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '12px 0', display: 'flex', flexDirection: 'column', gap: '4px' }}>
        {/* Business Name */}
        <div style={{ fontWeight: 400, fontSize: '15px', color: '#0066CC', lineHeight: 1.3, cursor: 'pointer' }}>
          {vendor.BusinessName || vendor.name}
        </div>
        
        {/* Price, Rating, Category, Response Time */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap', fontSize: '14px' }}>
          {hourlyRate > 0 && <span style={{ fontWeight: 600, color: '#222' }}>${hourlyRate}/hr</span>}
          ...
          {rating > 0 && (
            <span style={{ display: 'flex', alignItems: 'center', gap: '3px' }}>
              ...
              <span style={{ fontWeight: 600, color: '#222' }}>{rating.toFixed(2)}</span>
              {reviewCount > 0 && <span style={{ color: '#717171' }}>({reviewCount})</span>}
            </span>
          )}
          {primaryCategory && (rating > 0 || hourlyRate > 0) && <span style={{ color: '#717171' }}>•</span>}
          {primaryCategory && (
            <span style={{ display: 'flex', alignItems: 'center', gap: '4px', color: '#717171' }}>
              <span dangerouslySetInnerHTML={{ __html: categoryIconHtml }} />
          ...
          {isPremium && (
            <span style={{ display: 'flex', alignItems: 'center', gap: '3px' }}>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" style={{ display: 'block', height: '13px', width: '13px', fill: '#FF8C00' }}>
                <path d="M8 1l-1.5 5H1l4 3.5L3 15l5-3.5L13 15l-2-5.5 4-3.5h-5.5L8 1z"/>
              </svg>
              <span style={{ fontWeight: 600, color: '#FF8C00' }}>Instant book</span>
            </span>
          )}
        </div>
