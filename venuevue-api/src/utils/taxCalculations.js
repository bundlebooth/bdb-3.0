/**
 * Tax Calculation Utilities (Backend)
 * Shared logic for province detection and tax calculation across the API
 */
 ...
 */
function calculateBookingFees(subtotal, eventLocation, commissionSettings = {}) {
  ...
  const tax = Math.round(taxableAmount * taxPercent * 100) / 100;
  const stripeFee = Math.round((subtotal * stripeFeePercent + stripeFeeFixed) * 100) / 100;
  const total = Math.round((subtotal + platformFee + tax + stripeFee) * 100) / 100;
