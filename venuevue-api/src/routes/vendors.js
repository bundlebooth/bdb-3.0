...
// Step 1: Get vendors by service categories only
router.get('/by-categories', async (req, res) => {
  try {
    ...
    // Build dynamic query for category filtering
    const categoryConditions = categoryArray.map((_, index) => `vc.Category = @Category${index}`).join(' OR ');
    
    categoryArray.forEach((category, index) => {
      request.input(`Category${index}`, sql.NVarChar(50), category);
    });
    ...
    const query = `
      SELECT DISTINCT
        ...
        v.Latitude,
        v.Longitude,
        v.FeaturedImageURL as featuredImage,
        v.AverageResponseTime,
        v.ResponseRate,
        v.PriceLevel,
        STRING_AGG(vc.Category, ', ') as categories
      FROM VendorProfiles v
      INNER JOIN VendorCategories vc ON v.VendorProfileID = vc.VendorProfileID
      WHERE v.AcceptingBookings = 1
        AND v.IsCompleted = 1
        AND v.IsVerified = 1
        AND (${categoryConditions})
      GROUP BY v.VendorProfileID, v.BusinessName, v.DisplayName, v.City, v.State, 
               v.Latitude, v.Longitude, v.FeaturedImageURL, v.AverageResponseTime, 
               v.ResponseRate, v.PriceLevel
    ...
    res.json({
      ...
      totalCount: enhancedVendors.length,
      searchCriteria: {
    ...
    res.status(500).json({ 
      success: false,
      message: 'Failed to filter vendors by categories',
      error: err.message 
    });
  }
});

// Step 2: Filter vendors by availability and location
router.get('/filter-availability', async (req, res) => {
  try {
    const {
      vendorIds, // Comma-separated list from Step 1
      date,
      time,
      latitude,
      longitude,
    ...
    if (!vendorIds) {
      return res.status(400).json({
        success: false,
        message: 'VendorIds parameter is required'
      });
    }
    ...
    // Parse vendor IDs
    const vendorIdArray = vendorIds.split(',').map(id => parseInt(id.trim()));
    const vendorIdConditions = vendorIdArray.map((_, index) => `v.VendorProfileID = @VendorId${index}`).join(' OR ');
    
    vendorIdArray.forEach((vendorId, index) => {
      request.input(`VendorId${index}`, sql.Int, vendorId);
    });

    // Add other parameters
    request.input('EventDate', sql.Date, date ? new Date(date) : null);
    request.input('EventTime', sql.Time, time || null);
    ...
    const query = `
      ...
      FROM VendorProfiles v
      INNER JOIN VendorCategories vc ON v.VendorProfileID = vc.VendorProfileID
      LEFT JOIN VendorBusinessHours vbh ON v.VendorProfileID = vbh.VendorProfileID
      LEFT JOIN VendorServiceAreas vsa ON v.VendorProfileID = vsa.VendorProfileID
      WHERE v.AcceptingBookings = 1
        AND v.IsCompleted = 1
        AND v.IsVerified = 1
        AND (${vendorIdConditions})
        -- Business hours filtering (if time provided)
        AND (@EventTime IS NULL OR @EventDate IS NULL OR 
             vbh.IsAvailable = 1 AND 
             vbh.DayOfWeek = DATEPART(WEEKDAY, @EventDate) - 1 AND
             (@EventTime >= vbh.OpenTime AND @EventTime <= vbh.CloseTime))
        -- Location filtering: either within business location radius OR in service area
        AND (@Latitude IS NULL OR @Longitude IS NULL OR
             -- Within business location radius
             ...
             EXISTS (
               SELECT 1 FROM VendorServiceAreas vsa2 
               WHERE vsa2.VendorProfileID = v.VendorProfileID 
               AND vsa2.RadiusMiles >= 3959 * ACOS(
                 COS(RADIANS(@Latitude)) * COS(RADIANS(vsa2.Latitude)) * 
                 COS(RADIANS(vsa2.Longitude) - RADIANS(@Longitude)) + 
                 SIN(RADIANS(@Latitude)) * SIN(RADIANS(vsa2.Latitude))
               )
             ))
      GROUP BY v.VendorProfileID, v.BusinessName, v.DisplayName, v.City, v.State, 
               v.Latitude, v.Longitude, v.FeaturedImageURL, v.AverageResponseTime, 
               v.ResponseRate, v.PriceLevel
      ORDER BY distance ASC, v.BusinessName
    `;
    ...
    console.error('Availability filtering error:', err);
    res.status(500).json({ 
      success: false,
      message: 'Failed to filter vendors by availability',
      error: err.message 
    });
  }
});
