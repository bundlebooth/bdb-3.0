...
router.get('/:id', async (req, res) => {
  try {
    ...
    // Handle conditional recordsets based on whether UserID was provided
    const hasUserIdRecordset = userId !== null && userId !== undefined;
    const favoriteRecordsetIndex = hasUserIdRecordset ? 11 : -1;
    const slotsRecordsetIndex = hasUserIdRecordset ? 12 : 11;

    const vendorDetails = {
      profile: result.recordsets[0][0],
      categories: result.recordsets[1],
      services: result.recordsets[2],
      portfolio: result.recordsets[3],
      reviews: result.recordsets[4],
      faqs: result.recordsets[5],
      team: result.recordsets[6],
      socialMedia: result.recordsets[7],
      businessHours: result.recordsets[8],
      images: result.recordsets[9],
      categoryAnswers: result.recordsets[10],
      isFavorite: hasUserIdRecordset && result.recordsets[favoriteRecordsetIndex] ? 
                  result.recordsets[favoriteRecordsetIndex][0].IsFavorite : false,
      availableSlots: result.recordsets[slotsRecordsetIndex] || []
    };
